...
# ==========================================
# 1. CORE SIMULATION ENGINES (MANUAL LOGIC)
#===========================================
...
# ==========================================
# 2. UI APPLICATION CLASS WITH COMPARISON
# ==========================================

class MultiAlgorithmSimulator:
    def __init__(self, root):
        self.root = root
        self.root.title("OS Page Replacement Simulator - Comparison Edition")
    ...
    def setup_ui(self):
        title_lbl = tk.Label(self.root, text="Page Replacement Algorithms Simulator",
                             font=("Helvetica", 16, "bold"), bg="#f4f6f9", fg="#2c3e50")
        title_lbl.pack(pady=5)

        input_frame = tk.LabelFrame(self.root, text=" Simulation Settings ", font=("Helvetica", 10, "bold"),
                                    bg="#f4f6f9", fg="#34495e", padx=10, pady=5)
        input_frame.pack(fill="x", padx=15, pady=5)

        tk.Label(input_frame, text="Active Algorithm:", bg="#f4f6f9", font=("Helvetica", 9, "bold")).grid(row=0,
                                                                                                          column=0,
                                                                                                          sticky="w",
                                                                                                          pady=2)
        self.algo_combo = ttk.Combobox(input_frame, values=["LRU (Least Recently Used)", "FIFO (First-In First-Out)",
                                                            "Optimal (OPT)"], state="readonly", width=25,
                                       font=("Helvetica", 9))
        self.algo_combo.current(0)
        self.algo_combo.grid(row=0, column=1, padx=5, pady=2, sticky="w")

        tk.Label(input_frame, text="Reference String:", bg="#f4f6f9", font=("Helvetica", 9)).grid(row=1, column=0,
                                                                                                  sticky="w", pady=2)

        self.str_entry = tk.Entry(input_frame, width=60, font=("Helvetica", 9), fg="gray")
        self.str_entry.insert(0, self.default_string)
        self.str_entry.grid(row=1, column=1, padx=5, pady=2, sticky="w")

        # ربط أحداث الماوس والتركيز بالدوال بالأسفل
        self.str_entry.bind("<FocusIn>", self.on_enter)
        self.str_entry.bind("<FocusOut>", self.on_leave)

        # خانة الـ Frames
        tk.Label(input_frame, text="Frames (1-7):", bg="#f4f6f9", font=("Helvetica", 9)).grid(row=2, column=0,
                                                                                              sticky="w", pady=2)
        self.frames_entry = tk.Entry(input_frame, width=8, font=("Helvetica", 9), fg="gray")
        self.frames_entry.insert(0, "3")
        self.frames_entry.grid(row=2, column=1, padx=5, pady=2, sticky="w")

        # ربط أحداث الماوس والتركيز بخانة الـ Frames
        ...
        tk.Button(btn_frame, text="⚙️ Initialize Simulator", command=self.initialize_sim, bg="#34495e", fg="white",
                  font=("Helvetica", 9, "bold"), padx=10).pack(side="left", padx=3)
        self.prev_btn = tk.Button(btn_frame, text="⬅️ Prev", command=self.prev_step, bg="#e67e22", fg="white",
                                  font=("Helvetica", 9, "bold"), padx=10, state="disabled")
        self.prev_btn.pack(side="left", padx=3)
        self.next_btn = tk.Button(btn_frame, text="Next ➡️", command=self.next_step, bg="#2ecc71", fg="white",
                                  font=("Helvetica", 9, "bold"), padx=10, state="disabled")
        self.next_btn.pack(side="left", padx=3)

        self.comp_btn = tk.Button(btn_frame, text="📊 Show Quick Comparison", command=self.show_comparison, bg="#9b59b6",
                                  fg="white", font=("Helvetica", 9, "bold"), padx=10, state="disabled")
        self.comp_btn.pack(side="left", padx=3)

        self.display_canvas = tk.Canvas(self.root, bg="white", bd=1, relief="ridge")
        self.display_canvas.pack(fill="both", expand=True, padx=15, pady=5)

        self.compare_frame = tk.LabelFrame(self.root, text=" 📊 Live Multi-Algorithm Comparison Table ",
                                           font=("Helvetica", 10, "bold"), bg="#f4f6f9", fg="#2c3e50", pady=5)
        self.compare_frame.pack(fill="x", padx=15, pady=5)

        headers = ["Algorithm", "Page Faults ❌", "Fault Ratio", "Page Hits ✅", "Hit Ratio"]
        for col_idx, text in enumerate(headers):
            lbl = tk.Label(self.compare_frame, text=text, font=("Helvetica", 9, "bold"), bg="#dcdde1", fg="#2f3640",
        ...
        for row_idx, algo_name in enumerate(algos_keys, start=1):
            ...
            lbl_name = tk.Label(self.compare_frame, text=algo_name, font=("Helvetica", 9, "bold"), bg="#f5f6fa",
                                anchor="center", width=18, bd=1, relief="solid")
            lbl_name.grid(row=row_idx, column=0, padx=2, pady=1)

            for col_idx in range(1, 5):
                lbl_val = tk.Label(self.compare_frame, text="-", font=("Helvetica", 9), bg="white", width=18, bd=1,
                                   relief="solid")
                lbl_val.grid(row=row_idx, column=col_idx, padx=2, pady=1)
                self.rows_ui[algo_name].append(lbl_val)

        # تم إصلاح القوس الممسوح هنا وتظبيطه تماماً
        self.status_lbl = tk.Label(self.root, text="Select parameters and click 'Initialize'",
                                   font=("Helvetica", 10, "italic"), bg="#f4f6f9", fg="#7f8c8d")
    ...
    def initialize_sim(self):
        if self.str_entry.cget("fg") == "gray" or self.frames_entry.cget("fg") == "gray":
            messagebox.showwarning("warning ", "Enter Number Of Frames and Refrence String Correct!")
            return

        try:
            # قراءة البيانات الحقيقية اللي المستخدم كتبها بإيده طالما الخط بقى أسود
            ...
            messagebox.showerror("Error", "Please enter a valid reference string and frames (1-7).")
            return
        ...
        if "LRU" in selected_algo:
            self.frames_history, self.faults_history, self.victim_history = PageReplacementEngines.run_lru(self.pages,
                                                                                                           self.num_frames)
        elif "FIFO" in selected_algo:
            self.frames_history, self.faults_history, self.victim_history = PageReplacementEngines.run_fifo(self.pages,
                                                                                                            self.num_frames)
        elif "Optimal" in selected_algo:
            self.frames_history, self.faults_history, self.victim_history = PageReplacementEngines.run_optimal(
                self.pages, self.num_frames)
    ...
    def show_comparison(self):
        ...
        self.status_lbl.config(text="Comparison Table Updated for all 3 algorithms below! 📊", fg="#27ae60")
    ...
    def update_row_values(self, algo_key, faults, f_ratio, hits, h_ratio):
        ...
        bg_col = "#e8f4f8" if algo_key == active_algo_short or (
                algo_key == "Optimal" and active_algo_short == "Optimal(OPT)") else "white"
    ...
    def draw_grid(self):
        ...
        start_x, start_y, cell_size = 40, 45, 30

        self.display_canvas.create_text(start_x + 50, start_y, text="Ref String:", font=("Helvetica", 9, "bold"),
                                        anchor="e")
        for i in range(self.num_frames):
            self.display_canvas.create_text(start_x + 50, start_y + 35 + i * cell_size, text=f"Frame {i + 1}:",
                                            font=("Helvetica", 9, "bold"), anchor="e")
        self.display_canvas.create_text(start_x + 50, start_y + 45 + self.num_frames * cell_size, text="Status:",
                                        font=("Helvetica", 9, "bold"), anchor="e")
        ...
        for step in range(self.current_step):
            ...
            self.display_canvas.create_text(x + cell_size / 2, start_y, text=str(self.pages[step]), font=font_style,
                                            fill=text_color)
            ...
            for f_idx in range(self.num_frames):
                ...
                if is_current:
                    if is_fault and f_idx == victim_idx:
                        bg_color = "#ffcccc"
                    elif not is_fault and f_idx < len(current_f) and current_f[f_idx] == self.pages[step]:
                        bg_color = "#d4edda"

                self.display_canvas.create_rectangle(x, y, x + cell_size, y + cell_size, fill=bg_color,
                                                     outline="#bdc3c7")

                if f_idx < len(current_f):
                    val_color = "#c0392b" if (is_current and is_fault and f_idx == victim_idx) else "black"
                    self.display_canvas.create_text(x + cell_size / 2, y + cell_size / 2, text=str(current_f[f_idx]),
                                                    font=("Helvetica", 9, "bold"), fill=val_color)

            status_y = start_y + 35 + self.num_frames * cell_size
            status_text, status_color = ("F", "#e74c3c") if is_fault else ("H", "#2ecc71")
            self.display_canvas.create_text(x + cell_size / 2, status_y, text=status_text,
                                            font=("Helvetica", 10, "bold"), fill=status_color)
